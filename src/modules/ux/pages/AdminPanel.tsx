        ...
        <div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {logs.map((log: any, i: number) => {
              const colors: Record<string, string> = { auth: '#42a5f5', ai: '#7c4dff', error: '#ef5350' };
              const color = colors[log.type] || '#fff';
              return (
                <div key={i} style={{ display: 'flex', gap: '1rem', padding: '0.75rem 1rem', borderRadius: '8px', background: 'var(--bg-secondary)', border: `1px solid ${color}22` }}>
                  <span style={{ color: 'var(--text-secondary)', fontSize: '0.78rem', fontFamily: 'monospace', flexShrink: 0 }}>{log.time}</span>
                  <span style={{ background: `${color}22`, color: color, padding: '0.1rem 0.5rem', borderRadius: '4px', fontSize: '0.72rem', fontWeight: 'bold', flexShrink: 0, alignSelf: 'flex-start' }}>
                    {log.typeLabel}
                  </span>
